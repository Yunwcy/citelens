...
12 個測試問題，涵蓋 7 個章節、中英文各半。判準是「該讀的那一節有沒有被檢索到」——
分母取 `TOP_K = 8`，也就是系統實際送進模型的片段數。以系統自身做消融：

| 設定 | 前 8 名（實際送進模型） | 前 3 名（更嚴格） |
|---|---:|---:|
| 基準：固定視窗切塊 · 純向量檢索 · 不處理表格 | **0%** | **0%** |
| ＋結構感知切塊 | **83%** | **75%** |
| 完整系統：＋混合檢索 ＋表格處理 | **100%**（12/12） | **83%** |

兩個判準都列出來，不挑對自己有利的：前 8 名對應系統的實際行為，
前 3 名是更嚴格的視角。逐題名次見 [`docs/results/retrieval.md`](docs/results/retrieval.md)。
...
單次查詢平均 US$0.000847，脈絡用量中位數 2,736 / 6,000 tokens；
