...
`e2e.py` 裡有兩種性質完全不同的檢查，混為一談會導致「重跑到綠」這種
把問題洗掉的做法：

| 類別 | 例子 | 紅燈代表 |
|---|---|---|
| **確定性檢查** | 表格數值守恆、鍵唯一、脈絡未超預算、離線索引成功、重啟後資料保存 | 系統真的有缺陷，重跑無意義 |
| **模型輸出檢查** | `③ 消融　結論未做全稱誤述`、`② 比較　Comprehensiveness 未被誤稱為全勝`、`每個變體在表格之外都有敘述` | 模型在該次生成中真的寫錯了 |

第二類即使溫度為 0 仍會偶發，因為檢索平手時的順序、以及背景摘要是否已完成，
都會改變送進模型的脈絡。**實測 `③ 消融　結論未做全稱誤述` 在同一份文件、
同一個問題下連跑 6 次，失敗 1 次**，失敗時的句子是
`outperforms its ablated variants across all datasets and metrics` ——
而同一則回答所附的消融表格裡，48 組比較有 8 組是變體優於完整版，該句與證據矛盾。

這一項刻意保留成會偶發失敗的檢查，而不是放寬判準或移除：

- 它量到的是真實發生率，不是「有沒有可能發生」
- 提示詞層級的禁止對這類全稱說法不可靠 —— 比較路由就是因此改為
  預先算好統計再交給模型，而非要求模型自我克制
- 重跑通過只代表該次沒有發生，不代表問題不存在
